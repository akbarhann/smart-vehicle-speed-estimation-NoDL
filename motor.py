...
initial_roi = np.load("test.npy").astype(np.float32).reshape(-1, 1, 2)
...
bgsub = cv2.createBackgroundSubtractorMOG2(history=800, varThreshold=16, detectShadows=False,)
bgsub.setBackgroundRatio(0.3)
...
while True:
    ...
    exit_pt1, exit_pt2 = sorted_pts[0], sorted_pts[1]  # Sekarang ungu = exit
    entry_pt1, entry_pt2 = sorted_pts[2], sorted_pts[3]
    ...
    for objectID, times in crossing_times.items():
        # Pastikan entry_point ada
        if 'entry_point' in times:
            entry_pt = tuple(np.int32(times['entry_point']))
            cv2.circle(bird_overlay, entry_pt, 5, (255, 0, 0), -1)  # Titik biru di birdview

            # Jika exit_point juga ada, gambar garis
            if 'exit_point' in times:
                    exit_pt = tuple(np.int32(times['exit_point']))
                    cv2.circle(bird_overlay, exit_pt, 5, (0, 0, 255), -1)  # Titik merah
                    cv2.line(bird_overlay, entry_pt, exit_pt, (255, 255, 255), 1)

            # Gambar ID hanya jika entry_point tersedia
        ...
        if 'entry' in crossing_times[objectID] and 'exit' not in crossing_times[objectID]:
            if intersect(previous_pos, current_pos, exit_pt1, exit_pt2):
                ...
                if time_seconds > 0:
                    ...
                    real_world_distance_m = 12.0
                    pixel_to_meter_bird = real_world_distance_m / np.linalg.norm(np.array(exit_bird) - np.array(entry_bird))
    ...
    cv2.line(frame, tuple(entry_pt1.astype(int)), tuple(entry_pt2.astype(int)), (0, 255, 255), 2)  # kuning (entry)
    cv2.line(frame, tuple(exit_pt1.astype(int)), tuple(exit_pt2.astype(int)), (255, 0, 255), 2)  # ungu (exit)
